...
for it_co=1:size(TASKS, 2)
    for it_su=1:size(SUB_LIST, 2)
        ...
        % Load data
        res = load(fullfile(pathname, ['sub-', sub_ID, '\tc', '\sub-', sub_ID, '_demeanDetr_filt_1_', TASKS{it_co}, '_model.mat']));
        ...
        if iscell(res.results.LIN_HOPF_INDIV.errorFC)
            ...
            saveas(gcf, fullfile(pathname, ['sub-', sub_ID, '\tc\ModelResults_', TASKS{it_co}, '.jpeg']), 'jpeg');
        else
            n_blocks = 1;
