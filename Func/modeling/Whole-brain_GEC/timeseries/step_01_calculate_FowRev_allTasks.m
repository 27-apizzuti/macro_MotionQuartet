...
for sub=1:NSUB
    ...
    path_tc = fullfile(path_sbj, ['sub-' sub_ID '_' TASK '_VOICarpet.nii.gz']);
    ...
    for it=1:n_runs
        ...
        % disp(['Run FCtf ' num2str(it), 'mean: ' num2str(nanmean(FCtf(:))), 'var: ', num2str(std(FCtf(:))), 'min: ' num2str(min(FCtf(:))), 'max: ', num2str(max(FCtf(:)))])
        % disp(['Run NR ' num2str(it), 'mean: ' num2str(nanmean(Reference)), 'var: ', num2str(std(Reference)), 'min: ' num2str(min(Reference)), 'max: ', num2str(max(Reference))])
        ...
        clear FCtf FCtr Itauf Itaur Reference ts
    end

    % Save time course as .mat file
    save(fullfile(path_sbj_out, ['sub-' sub_ID '_' TASK '_VOICarpet_demeanDetr_filt_', num2str(butterworth), '.mat']), 'temp_sig_filt');

    % Save as carpet fMRI data after filtering
    temp_sig_filt = reshape(temp_sig_filt, size(data_tc));
    outputname = fullfile(path_sbj_out, ['sub-' sub_ID '_' TASK '_VOICarpet_demeanDetr_filt_', num2str(butterworth), '.nii.gz']);
    niftiwrite(temp_sig_filt, outputname, info_tc)
    
    % Save single subject results
    save(fullfile(path_sbj_out, ['sub-', sub_ID, '_' TASK ' _runs_demeanDetr_filt_', num2str(butterworth), '_', TASK,  'NonRever.mat']), 'FowRev_matrix');
