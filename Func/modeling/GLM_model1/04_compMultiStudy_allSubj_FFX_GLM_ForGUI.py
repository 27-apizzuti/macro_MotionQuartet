...
TASKS = ['phy', 'amb']
PATH_GLM = os.path.join(STUDY_PATH, 'GLM_model_2pred')
...
for task in TASKS:
    ...
    for su in SUBJ:
        ...
        PATH_SDM = os.path.join(STUDY_PATH, su, 'func', 'GLM_model_2pred', 'SDM')
    ...
    doc.psc_transform_studies = False
    doc.z_transform_studies = True
    doc.z_transform_studies_baseline_only = False
    doc.separate_subject_predictors = True
    doc.separate_study_predictors = False
    doc.save_multistudy_glm_definition_file(os.path.join(PATH_GLM, 'AllSbj_{}_2preds_MOCO_ZT_SEPSTUDYPREDS_N-{}_FFX.mdm'.format(task, len(SUBJ))))
