...
for su in SUBJ:
    ...
    PATH_GLM = os.path.join(STUDY_PATH, 'derivatives', su, 'func', 'GLM_model4')
    ...
    for ru in RUNS:
        ...
        if task == 'phy':
            PATH_PROTOCOL = os.path.join(STUDY_PATH, 'Protocols', su, 'Exp1_Phys_MotQuart', 'Protocols', 'Protocol_{}_Exp2_unamb_MotQuart_Run{}_model4.prt'.format(su, runID))
        else:
            PATH_PROTOCOL = os.path.join(STUDY_PATH, 'Protocols', su, 'Exp1_Amb_MotQuart', 'Protocols', 'Protocol_{}_Protocols_sess-03_Run{}_model4.prt'.format(su, runID))
        ...
        docVMR.add_predictor("Sustained")
        docVMR.set_predictor_values_from_condition("Sustained", "Sustained", 1)
        docVMR.apply_hrf_to_predictor("Sustained")

        if task == 'amb':
            # // Add nuisance predictors
        ...
        outname = os.path.join(PATH_OUT, '{}_{}_model4.sdm'.format(su, ru))
        ...
        hdr['IncludesConstant'] = 1
        if task == 'amb':
            hdr['FirstConfoundPredictor'] = len(new_data) - 7
        else:
            hdr['FirstConfoundPredictor'] = len(new_data) - 6
        outname = os.path.join(PATH_OUT, '{}_{}_model4_MOCO.sdm'.format(su, ru))
