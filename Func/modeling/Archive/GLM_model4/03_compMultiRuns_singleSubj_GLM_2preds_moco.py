...
for su in SUBJ:
    ...
    PATH_GLM = os.path.join(STUDY_PATH, 'derivatives', su, 'func', 'GLM_model4', 'MultiRuns')
    PATH_VTC = os.path.join(STUDY_PATH, 'derivatives', su, 'func', 'VTC_MNI')
    PATH_SDM = os.path.join(STUDY_PATH, 'derivatives', su, 'func', 'GLM_model4', 'SDM')
    ...
    for task in TASKS:
        ...
        for itrun in RUNS:
            ...
            sdm = os.path.join(PATH_SDM, '{}_{}0{}_model4_MOCO.sdm'.format(su, task, itrun))
            doc.add_study_and_dm(vtc_file, sdm)

        doc.save_multistudy_glm_definition_file(os.path.join(PATH_GLM, '{}_{}_allRuns_MOCO.mdm'.format(su, task)))
        ...
        doc.save_glm(os.path.join(PATH_GLM, '{}_{}_allRuns_MOCO_AR2_PSC_SSP_N-{}.glm'.format(su, task, len(RUNS))))
        doc.show_glm()
        doc.close()
