...
SUBJ = ["sub-06", "sub-07", "sub-08", "sub-09", "sub-10", "sub-03", "sub-05"]
...
for su in SUBJ:
    ...
    VTC_PATH = os.path.join(STUDY_PATH, su, "VTC_native".format(su))
    PROT_PATH = os.path.join(STUDY_PATH, su, "Protocol".format(su))
    PATH_GLMSINGLE = os.path.join(STUDY_PATH, su, 'GLMsingle-v02')
    ...
    # PATH_GLMSINGLE = os.path.join(STUDY_PATH, "derivatives", su, "func", 'GLMsingle-v02')
    ...
    for it_run in range(0, N_RUNS):
        ...
        # Horizontal
        n = len(prt_data[1]['Time start'])
        for i in range(0, n):
            xstart = prt_data[1]['Time start'][i]-1
            xend = prt_data[1]['Time stop'][i]
            design_run[xstart:xend, 0] = 1

        # Vertical
        n = len(prt_data[2]['Time start'])
        for i in range(0, n):
            xstart = prt_data[2]['Time start'][i]-1
            xend = prt_data[2]['Time stop'][i]
            design_run[xstart:xend, 1] = 1
        ...
        # -----
        # TRIM DESIGN MATRIX: Remove start and end fixation
        design_run_trim = design_run[20:597]
    ...
    for run in range(1,N_RUNS+1):
        ...
        sdm.write_sdm(os.path.join(PATH_GLMSINGLE, "GLMdenoise_nuisance_regressors_run{}.sdm".format(run)), hdr, sdm_data)
